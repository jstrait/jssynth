...
class Controls extends React.PureComponent {
  ...
  render() {
    return <span className="whitespace-wrap-none">
      <button className="button-tab-list button-standard" onClick={this.props.onRewind}><span className="rewind-icon">Rewind</span></button>
      <button className={"mr-half mr1-l button-tab-list button-standard" + (this.props.isPlaying ? " button-toggled" : "")}  onClick={this.props.onTogglePlayback}><span className={"play-icon" + (this.props.isPlaying ? " play-icon-enabled" : "")}>Play</span></button>
    </span>;
  };
};
