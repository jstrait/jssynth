...
export class DownloadButton extends React.PureComponent {
  ...
  render() {
    ...
    if (this.props.isEnabled === true) {
      bodyContent = <span>
        ...
        <div className="mt1 flex flex-justify-end flex-align-center">
          ...
          <button className="button-standard button-standard-full" disabled={this.state.isDownloadInProgress} onClick={this.beginDownload}>Download</button>
        </div>
      </span>;
    }
    else {
    ...
    return <div id="download-container" className="relative flex flex-justify-end">
      <button className="button-standard button-standard-full flex flex-align-center" onClick={this.togglePopup}>
